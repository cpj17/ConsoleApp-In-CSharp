    ...
    class Program
    {
        ...
        static void login(DataSet objDataSetUser)
        {
            ...
            if (objDataRowUsers[0]["Password"].ToString() == password)
            {
                ...
                Int64 objInt64SessionId = Convert.ToInt64(objDateTime.Day.ToString() + objDateTime.Month.ToString() + objDateTime.Year.ToString() + objDateTime.Hour.ToString() + objDateTime.Minute.ToString() + objDateTime.Second.ToString());
                //Console.WriteLine(objInt64SessionId);

                objDataSetUser.Tables[1].Rows.Add(username, objDateTime, objInt64SessionId);
                ...
                {
                    objDataSetUser.Tables[2].Rows.Add(objInt64SessionId, objArrayListUserComments[i]);
        ...
        static void loginHistory(DataSet objDataSetUser)
        {
            ...
            if (objDataRowUser[0]["Password"].ToString() == password)
            {
                ...
                DataRow objDataRowTable2Parent, objDataRowTable3Parent;

                DataRow[] objDataRowTable1Child = objDataSetUser.Tables[1].Select("Username='" + username + "'");
                objDataRowTable2Parent = objDataRowTable1Child[0].GetParentRow(objDataRelation1);

                for (int i = 0; i < objDataRowTable1Child.Count(); i++)
                {
                    Console.WriteLine("Time Stamp : "+objDataRowTable1Child[i][1]);
                    Console.WriteLine("Session ID : " + objDataRowTable1Child[i][2]);
                    
                    DataRow[] objDataRowTable2Child = objDataSetUser.Tables[2].Select("SessionId = '" + objDataRowTable1Child[i][2] + "'");
                    objDataRowTable3Parent = objDataRowTable2Child[0].GetParentRow(objDataRelation2);
                    //Console.WriteLine(objDataRowTable2Child.Count());

                    for (int j = 0; j < objDataRowTable2Child.Count(); j++)
                    {
                        Console.WriteLine(objDataRowTable2Child[j][1]);
                    }
                    Console.WriteLine();
                }
            }
